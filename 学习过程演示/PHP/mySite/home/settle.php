...
<head>
	...
	<link rel="stylesheet" href="public/css/settle.css">
	...
	<section>
		<div class="main">
			...
			<div class="shopbottom">
				...
				<div class="info">
					<?php 
					$sqlinfo="select address.* from user,address where address.user_id=user.id&&user.username='{$_COOKIE['user']}'&&address.isdefault=1";
					$smtinfo=$pdo->prepare($sqlinfo);
					$smtinfo->execute();
					$rowinfo=$smtinfo->fetch();
					 ?>
					<form action="">
						<p>确认收货地址</p>
						<div>
							<label>收货人姓名:</label>
							<input type="text" value="<?php echo $rowinfo['name'] ?>" name="name">
						</div>
						<div>
							<label>收货人手机:</label>
							<input type="text" value="<?php echo $rowinfo['phone'] ?>" name="phone">
						</div>
						<div>
							<label>收货人地址:</label>
							<input type="text" value="<?php echo $rowinfo['address'] ?>" name="address">
						</div>
						<div>
							<label>订单备注:</label>
						...
						<p>确认快递方式</p>
						<div>
							<input type="radio" value="" name="express">顺丰快递
							<input type="radio" value="" name="express">圆通快递
							<input type="radio" value="" name="express">中通快递
							<input type="radio" value="" name="express">申通快递
						</div>
						<div class="submit">
							<div>
								<a href="">返回</a>
