...
//分页效果实现
$sql1="select count(*) from advertising";
...
$sql="select * from advertising order by place limit {$page->idx},{$page->num}";
$smt=$pdo->prepare($sql);
$smt->execute();
$rows=$smt->fetchAll();
 ?>
        	...
        	<table class="table table-striped table-bordered table-hover">
				...
				<tr>
					...
					foreach($rows as $row){
						echo "<tr>";
						echo "<td>{$row['id']}</td>";
						echo "<td><img src='/mySite/public/uploads/advertising/th_".$row['img']."'/></td>";
						echo "<td>{$row['place']}号位</td>";
						echo "<td>{$row['url']}</td>";
