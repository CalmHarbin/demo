...
//分页效果实现
$sql1="select count(*) from pricesection";
...
$sql="select * from pricesection order by id limit {$page->idx},{$page->num}";
$smt=$pdo->prepare($sql);
$smt->execute();
$rows=$smt->fetchAll();
 ?>
...
<body>
	<div class="thumbnail">
			...
			<p class="pull-right" id="sum">大分类个数:<span><?php echo $sum; ?></span></p>
			<?php 
				echo $page->show;
			 ?>
    </div>
