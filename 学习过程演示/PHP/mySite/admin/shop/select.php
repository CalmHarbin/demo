...
$sql="select * from shop order by id limit {$page->idx},{$page->num}";
$smt=$pdo->prepare($sql);
$smt->execute();
$rows=$smt->fetchAll();
...
<body>
	<div class="thumbnail">
        	<table class="table table-striped table-bordered table-hover">
				...
				<tr>
					...
					foreach($rows as $row){
						...
						echo "<td>{$row['id']}</td>";
						echo "<td>{$row['name']}</td>";
						echo "<td><img src='/mySite/public/uploads/shop/th_".$row['img']."'/></td>";
						echo "<td>{$row['currentPrice']}</td>";
						echo $row['goodsShelf']?"<td>上架</td>":"<td>下架</td>";
						echo "<td>{$rowclass['lname']}-{$rowclass['bname']}</td>";
			...
			<p class="pull-right" id="sum">商品个数:<span><?php echo $sum; ?></span></p>
			<?php 
				echo $page->show;
			 ?>
    </div>
