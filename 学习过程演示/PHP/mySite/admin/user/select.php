...
//分页效果实现
$sql1="select count(*) from user where isadmin=0";
...
$sql="select * from user where isadmin=0 order by id desc limit {$page->idx},{$page->num}";
$smt=$pdo->prepare($sql);
$smt->execute();
$rows=$smt->fetchAll();
 ?>
...
<body>
	<div class="thumbnail">
			...
			<p class="pull-right" id="sum">当前会员人数:<span><?php echo $sum; ?></span></p>
			<?php 
				echo $page->show;
			 ?>
    </div>
